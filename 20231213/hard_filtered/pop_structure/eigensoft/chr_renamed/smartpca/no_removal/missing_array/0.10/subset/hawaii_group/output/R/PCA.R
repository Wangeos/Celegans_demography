...
df_pcs1 <- data.table::fread("../../pca.evec", skip = 1)
eval <- data.table::fread("../../pca.eval")
...
iso_table <- read_xlsx("C:/Users/wangc/Downloads/Research/PhD/Data_analyses/Caenorhabditis/Celegans/CaeNDR/20231213/raw/20231213_c_elegans_strain_data-summary.xlsx",sheet="isotypes")
...
iso_table_new <- read_xlsx("C:/Users/wangc/Downloads/Research/PhD/Data_analyses/Caenorhabditis/Celegans/CaeNDR/20231213/geo_info/iso_table_geo.xlsx",sheet="iso_table_geo")
...
plot_PC12_hawaii_g_ellipse_poster <- ggplot(df_pcs_geo_loc_new_supple) +
  geom_mark_ellipse(aes(x=PC1, y=PC2,
                        fill = collection_cluster_3km), 
                    alpha = 0.3,linetype=0)+
  scale_fill_brewer("Sampling locations",palette = "Set2",)+
  scale_x_continuous(expand = expansion(mult = 0.2)) +
  scale_y_continuous(expand = expansion(mult = 0.2))+
  new_scale_fill() + 
  geom_point(size=2, alpha=0.8, aes(x=PC1, y=PC2, fill=origin_hawaii, shape=origin_lee)) +
  scale_fill_manual(values = cols_hawaii,
                    breaks = c("Africa","Asia","Atlantic","Australia",
                               ...
                               "Unknown"),
                    guide="none") +
  scale_shape_manual(values = shapes_new, guide="none") +
  theme_bw() +
  ...
  labs(x=paste0("PC1 (",round(eval$Proportion[1]*100,digits=2),"%)"), 
       y=paste0("PC2 (",round(eval$Proportion[2]*100,digits=2),"%)"))
plot_PC12_hawaii_g_ellipse_poster
ggsave("plots/PCA_hawaii_3K_ell_poster.pdf", height = 8*(sqrt(5)-1)/2, width = 8, dpi = 300)
...
plot_PC12_hawaii_g_ellipse_trail <- ggplot(df_pcs_geo_loc_new_supple) +
  geom_mark_ellipse(aes(x=PC1, y=PC2,
                        fill = collection_trail), 
                    alpha = 0.3,linetype=0)+
  scale_fill_brewer(palette = "Set2")+
  scale_x_continuous(expand = expansion(mult = 0.2)) +
  scale_y_continuous(expand = expansion(mult = 0.2))+
  ...
  labs(x=paste0("PC1 (",round(eval$Proportion[1]*100,digits=2),"%)"), 
       y=paste0("PC2 (",round(eval$Proportion[2]*100,digits=2),"%)"))+
  guides(fill= guide_legend(override.aes = list(shape=21),ncol=1))
plot_PC12_hawaii_g_ellipse_trail
...
df_pcs_geo_loc_new$jittered_x <- jitter(df_pcs_geo_loc_new$longitude, amount=5)
df_pcs_geo_loc_new$jittered_y <- jitter(df_pcs_geo_loc_new$latitude, amount=5)

pca_map_jitter <- world_map+
  geom_segment(aes(x = jittered_x, xend = longitude, y = jittered_y, yend = latitude), 
               data=df_pcs_geo_loc_new,
               color = "#555555",size=0.1) +
  geom_point(mapping=aes(x=jittered_x,y=jittered_y,fill=origin,shape=origin_lee), 
             data=df_pcs_geo_loc_new, 
             color="black", size=2)+
  scale_fill_manual(values = cols) +
  scale_shape_manual(values = shapes_new) +
  ...
  guides(fill="none")
pca_map_jitter
pdf("plots/PCA_map_jitter.pdf", width=12, height=6)
pca_map_jitter
dev.off()

df_pcs_geo_loc_new$jittered_x_hawaii <- jitter(df_pcs_geo_loc_new$longitude, amount=0.5)
df_pcs_geo_loc_new$jittered_y_hawaii <- jitter(df_pcs_geo_loc_new$latitude, amount=0.5)

pca_map_hawaii_jitter <- world_map+
  geom_segment(aes(x = jittered_x_hawaii, xend = longitude, y = jittered_y_hawaii, yend = latitude), 
               data=df_pcs_geo_loc_new,
               color = "#555555",size=0.1) +
  geom_point(mapping=aes(x=jittered_x_hawaii,y=jittered_y_hawaii,fill=origin_hawaii,shape=origin_lee), data=df_pcs_geo_loc_new, color="black", size=2)+
  scale_fill_manual(values = cols_hawaii) +
  scale_shape_manual(values = shapes_new) +
  theme_classic()+
  theme(legend.position = c(0.3,0.25),
  ...
  xlim(c(-161,-154))+ylim(c(18,23))+
  guides(fill="none")
pca_map_hawaii_jitter
pdf("plots/PCA_map_hawaii_jitter.pdf", width=6, height=6*(sqrt(5)-1)/2)
pca_map_hawaii_jitter
dev.off()

#Map the Big Island group (former Hawaiian group)
df_pcs_geo_loc_new$jittered_x_bi <- jitter(df_pcs_geo_loc_new$longitude, amount=0.2)
df_pcs_geo_loc_new$jittered_y_bi <- jitter(df_pcs_geo_loc_new$latitude, amount=0.2)
df_pcs_geo_loc_new$bi_group <- 
  df_pcs_geo_loc_new$PC1 < -0.01 & df_pcs_geo_loc_new$PC1 > -0.04 & df_pcs_geo_loc_new$PC2 < -0.08
#Check the count of the Big Island group isotypes
nrow(df_pcs_geo_loc_new[df_pcs_geo_loc_new$bi_group == TRUE,]) 
 
pca_map_bi_jitter <- world_map+
  geom_segment(aes(x = jittered_x_bi, xend = longitude, y = jittered_y_bi, yend = latitude, colour=bi_group), 
               data=df_pcs_geo_loc_new,
               size=0.1) +
  geom_point(mapping=aes(x=jittered_x_bi,y=jittered_y_bi,fill=origin_hawaii,shape=origin_lee), data=df_pcs_geo_loc_new, color="black", size=2)+
  scale_colour_manual(values = c("red","#555555"),
                      breaks = c("TRUE","FALSE"),
                      labels = c("Big Island group\n(former Hawaii group)","Others")) +
  scale_fill_manual(values = cols_hawaii) +
  scale_shape_manual(values = shapes_new) +
  theme_classic()+
  ...
  coord_sf()+
  xlab(NULL)+ylab(NULL)+
  xlim(c(-157,-154))+ylim(c(18.5,20.4))+
  guides(fill="none")
pca_map_bi_jitter
pdf("plots/PCA_map_big_island_jitter.pdf", width=6, height=6*(sqrt(5)-1)/2)
pca_map_bi_jitter
dev.off()
