...
population="Hawaii"
M=2 # Number of haploid genomes
NC=5 # Number of chromosomes
mu=2.3E-9 # Mutation rate
r=3.07652E-8  # Recombination rate

setwd("output/")

Os=Get_real_data("../../../../../05.generate_mhs/2samples/bi1/output/", M,	"hawaii_g.bi1.2samples.hap.mhs",delim = "\t")
results=eSMC2(n=40, rho=r/mu, Os, SF=T, Rho=F,
              BoxP=c(3,3),NC=length(Os))
...
ggsave("plots/pop_trajectory_log_1E3_poster.pdf",height = 7*(sqrt(5)-1)/2, width = 5, dpi = 300)
...
ggsave("plots/pop_trajectory_log_1E3_poster_sigma.pdf",
       height = 7*(sqrt(5)-1)/2, width = 5, dpi = 300, device = cairo_pdf)
